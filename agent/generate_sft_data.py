...
# ─────────────────────────────────────────────────────────────
# FAILURE MODE 5 FIX (SFT Data Poisoning):
# Diversity prompts prevent the teacher from developing a single
# rigid investigation pattern. Each episode gets a different
# approach suffix, ensuring the student model learns multiple
# valid solution strategies instead of one memorized path.
# ─────────────────────────────────────────────────────────────

DIVERSITY_STRATEGIES = [
    "Start by checking the overall system status to get a big picture view before drilling into any specific service.",
    "Start by checking the dependency graph first — understanding which services depend on which will guide your investigation.",
    "Start by checking the logs of the services that appear most unhealthy. The error messages will point you to the root cause faster.",
    "Start by checking metrics (CPU, memory, latency) across services. Resource exhaustion patterns are often the fastest diagnostic signal.",
    "Start by checking the most critical user-facing service first, then trace failures backward through the dependency chain to find the root cause.",
]
...
class ExpertEpisodeRunner:
    ...
    Each episode gets a randomly-selected investigation strategy
    (FM5 diversity fix) so the SFT dataset contains multiple
    valid solution approaches instead of one rigid pattern.
    """
    ...
    def _teacher_call(self, system_prompt: str, user_prompt: str) -> str:
        ...
        # FM5: Vary temperature between 0.6-0.9 for natural diversity
    ...
    def run_expert_episode(self, task_id: str) -> List[Dict[str, Any]]:
        ...
        # FM5: Select a random investigation strategy for this episode
        self._episode_count += 1
        self._current_strategy = DIVERSITY_STRATEGIES[self._episode_count % len(DIVERSITY_STRATEGIES)]
...
APPROACH GUIDANCE: {self._current_strategy}"""
    ...
    def _build_commander_prompt(
        ...
        return f"""Step {step_num}/20 | Last Reward: {last_reward:+.4f} | {phase}

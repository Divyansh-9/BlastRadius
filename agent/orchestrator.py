...
Recent History: {'; '.join(history[-3:]) if history else 'Episode start'}"""
    ...
    def run_commander(
        ...
        user_prompt = f"""Step {step_num}/20 | Last Reward: {last_reward:+.4f} | {phase}
...
Respond with <think>your reasoning</think> then <action>JSON</action>."""
    ...
    def run_episode(
        ...
        for step_num in range(1, max_steps + 1):
            ...
            # ── Execute Action ──
            # Coerce None target to "" for HTTP API compatibility
    ...
    def run_episode_stream(self, task_id: str, max_steps: int = 20):
        """
        ...
        for step_num in range(1, max_steps + 1):
            ...
            user_prompt = f"Step {step_num}/20 | {phase}\n\n[SCOUT TRIAGE REPORT]\n{triage}\n\n[EPISODE HISTORY]\n{chr(10).join(history[-5:]) if history else 'No actions taken yet.'}\n\nRespond with <think>your reasoning</think> then <action>JSON</action>."
            cmdr_full = ""

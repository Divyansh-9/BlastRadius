...
def compute_chain_similarity(
    ...
    """
    # Defensive: model may output non-list or non-string elements
    # e.g., causal_chain: "just a string" or [1, {"step": 2}]
    if isinstance(agent_chain, str):
        agent_chain = [agent_chain]
    agent_chain = [str(s) for s in agent_chain if s]
...
class Grader:
    ...
    def _grade_diagnosis_inner(self, params: Dict[str, Any]) -> tuple:
        ...
        # Defensive: model may output confidence as string ("0.8") or invalid type
        try:

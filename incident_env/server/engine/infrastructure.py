...
"
        ...
        if svc.fixable_by:
            ...
            return (
                f"⚠️ {svc.display_name} restarted but crashed again immediately.\
"
...
"
        ...
        if svc.fixable_by:
             return (
                f"⚠️ Rolled back {svc.display_name} to {svc.previous_version}, but service is still {svc.status.value}.\
"
    ...
    def scale_service(self, name: str, params: Dict) -> Tuple[str, bool]:
        ...
        if "scale" in svc.fixable_by:
            ...
            if svc.fix_params:
                ...
                if missing_or_wrong:
                    return (
                        f"⚠️ FAILED: Invalid or missing scaling parameters for {svc.display_name}. "
...
"
        ...
        if svc.fixable_by:
            return (
                f"⚠️ Resources adjusted for {svc.display_name}, but service remains {svc.status.value}.\
"

    ...
    def restart_service(self, name: str) -> Tuple[str, bool]:
        ...
        if "restart" in svc.fixable_by:
            ...
            if not ok:
                ...
                return (
                    f"?????? FAILED: Restarting {svc.display_name} while '{blocker}' is still "
        ...
        if svc.is_root_cause:
            return (
                f"?????? {svc.display_name} restarted but crashed again within 30 seconds.\n"
        ...
        return (
            f"?????? {svc.display_name} restarted but returned to {svc.status.value} "
    ...
    def rollback_deploy(self, name: str) -> Tuple[str, bool]:
        ...
        if "rollback" in svc.fixable_by:
            ...
            if not ok:
                self._apply_cascading_damage(name)
                return (
                    f"?????? FAILED: Rolling back {svc.display_name} while '{blocker}' "
    ...
    def scale_service(self, name: str, params: Dict) -> Tuple[str, bool]:
        ...
        if "scale" in svc.fixable_by:
            ...
            if not ok:
                self._apply_cascading_damage(name)
                return (
                    f"?????? FAILED: Scaling {svc.display_name} while '{blocker}' "
                    f"is unhealthy ??? resources allocated but service still failing."
                ), False

            # Validate required fix_params if the scenario defines them
            if svc.fix_params:
                missing_keys = [k for k in svc.fix_params if k not in params]
                if missing_keys:
                    return (
                        f"FAILED: scale_service on {svc.display_name} requires "
                        f"parameters {list(svc.fix_params.keys())}. "
                        f"Missing: {missing_keys}. Re-issue with correct parameters."
                    ), False
            svc.status = ServiceStatus.HEALTHY
            svc.current_metrics = copy.deepcopy(svc.healthy_metrics)
            svc.unhealthy_since_minute = -1
